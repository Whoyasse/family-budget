...
function CategoryLimitSheet({ category, monthLabel, spent, limit, percent, formatCurrency, onSave, onRemove, onClose }) {
  ...
  return <div className="sheet-backdrop" onClick={onClose}><section className="sheet-card category-limit-sheet" onClick={(event) => event.stopPropagation()}>
    ...
    <p className="muted">Лимит на {monthLabel}</p><p className="form-help">Лимит действует только для выбранного месяца.</p><div className="limit-sheet-summary"><strong>{formatCurrency(spent)}</strong>{limit ? <span>из {formatCurrency(limit)} · {percent.toFixed(0)}%</span> : <span>Лимит не задан</span>}</div>
    ...
    <label className="field"><span>Сумма лимита</span><input autoFocus type="number" min="0" inputMode="decimal" value={value} onChange={(event) => setValue(event.target.value)} placeholder="Например, 400" /></label>
    <button className="primary-btn" type="button" disabled={isSaving} onClick={async () => { if (isSaving) return; setIsSaving(true); try { await onSave(value); } finally { setIsSaving(false); } }}>{isSaving ? 'Сохраняем…' : 'Сохранить'}</button>{limit ? <button className="text-action danger" type="button" disabled={isSaving} onClick={async () => { if (isSaving) return; setIsSaving(true); try { await onRemove(); } finally { setIsSaving(false); } }}>Убрать лимит</button> : null}
  </section></div>;
}
